...
class TestCPU():
    ...
    def test_find_addressing_mode(self):
        for opcode in [0x0C, 0x0D, 0x0E, 0x0F, 0x20, 0x2C,
                        ...
                        0xEE, 0xEF]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ABSOLUTE
        for opcode in [0x1C, 0x1D, 0x1E, 0x1F, 0x3C, 0x3D, 0x3E,
            ...
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ABSOLUTE_X
        for opcode in [0x19, 0x1B, 0x39, 0x3B, 0x59,
                       0x5B, 0x79, 0x7B, 0x99, 0x9B,
                       0x9E, 0x9F, 0xB9, 0xBB, 0xBE,
                       0xBF, 0xD9, 0xDB, 0xF9, 0xFB]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ABSOLUTE_Y
        for opcode in [0x0A, 0x2A, 0x4A, 0x6A]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ACCUMULATOR
        for opcode in [0x09, 0x0B, 0x29, 0x2B, 0x49,
                       0x4B, 0x69, 0x6B, 0x80, 0x82,
                       0x89, 0x8B, 0xA0, 0xA2, 0xA9,
                       0xAB, 0xC0, 0xC2, 0xC9, 0xCB,
                       0xE0, 0xE2, 0xE9, 0xEB]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.IMMEDIATE
        for opcode in [0x01, 0x03, 0x21, 0x23, 0x41,
                       0x43, 0x61, 0x63, 0x81, 0x83,
                       0xA1, 0xA3, 0xC1, 0xC3, 0xE1,
                       0xE3]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.INDIRECT_X
        for opcode in [0x11, 0x13, 0x31, 0x33, 0x51,
                       0x53, 0x71, 0x73, 0x91, 0x93,
                       0xB1, 0xB3, 0xD1, 0xD3, 0xF1,
                       0xF3]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.INDIRECT_Y
        for opcode in [0x10, 0x30, 0x50, 0x70,
                       0x90, 0xB0, 0xD0, 0xF0]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.RELATIVE
        for opcode in [0x04, 0x05, 0x06, 0x07, 0x24, 0x25, 0x26, 0x27,
            ...
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ZERO_PAGE
        for opcode in [0x14, 0x15, 0x16, 0x17, 0x34, 0x35, 0x36, 0x37,
                       ...
                       0xF4, 0xF5, 0xF6, 0xF7]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ZERO_PAGE_X
        for opcode in [0x96, 0x97, 0xB6, 0xB7]:
            assert self.cpu_under_test.find_addressing_mode(opcode) == cpu.AddressingModes.ZERO_PAGE_Y
        assert self.cpu_under_test.find_addressing_mode(0x00) == cpu.AddressingModes.IMPLIED
    ...
    def test_decode_instruction(self):
        ...
        assert addressing_mode == cpu.AddressingModes.IMMEDIATE
        assert cost == 2

        opcode = 0x18 # CLC
        instruction, addressing_mode, cost = self.cpu_under_test.decode_instruction(opcode)
        assert instruction == self.cpu_under_test.CLC
        assert addressing_mode == cpu.AddressingModes.IMPLIED
        assert cost == 2
    ...
    def test_find_effective_address_imm(self):
        ...
        self.cpu_under_test.compute_effective_address(cpu.AddressingModes.IMMEDIATE)
    ...
    def test_find_effective_address_zp(self):
        ...
        self.cpu_under_test.compute_effective_address(cpu.AddressingModes.ZERO_PAGE) # EA <- RAM[PC+1]
        assert self.cpu_under_test.EA == self.cpu_under_test.read_RAM(0x4001) # .:. EA <- RAM[0x4001]
     
    def test_find_effective_address_zpx(self):
        # Zero-page, X mode. EA <- RAM[PC+1+X]
        for position in range(0, 65536):
            value = position & 0xFF
            self.cpu_under_test.write_RAM(position, value)
        ...
        self.cpu_under_test.compute_effective_address(cpu.AddressingModes.ZERO_PAGE_X) # EA <- RAM[PC+1+X]
    ...
    def test_find_effective_address_zpy(self):
        ...
        self.cpu_under_test.compute_effective_address(cpu.AddressingModes.ZERO_PAGE_Y) # EA <- RAM[PC+1+X]
        assert self.cpu_under_test.EA == self.cpu_under_test.read_RAM(target_EA)
    
    def test_find_effective_address_abs(self):
        # Absolute mode. EA <- RAM[PC, PC+1]    
        for position in range(0, 65536):
            value = position & 0xFF
            self.cpu_under_test.write_RAM(position, value)
        
        self.cpu_under_test.PC = 0x4040
        target_EA_low = self.cpu_under_test.read_RAM(self.cpu_under_test.PC)
        target_EA_high = self.cpu_under_test.read_RAM(self.cpu_under_test.PC + 1)
        target_EA = (target_EA_high << 8) | target_EA_low
        
        self.cpu_under_test.compute_effective_address(cpu.AddressingModes.ABSOLUTE)
        assert self.cpu_under_test.EA == target_EA
